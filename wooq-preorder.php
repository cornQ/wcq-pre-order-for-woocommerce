 ...
 */
function wooqpopskb_preorder_product_field() {
    wp_nonce_field( 'wooq_preorder_save', 'wooq_preorder_nonce' );
...
add_action( 'woocommerce_product_options_inventory_product_data', 'wooqpopskb_preorder_product_field' );

/**
 * Save the Pre-Order fields.
 */
function wooqpopskb_preorder_save_product_field( $post_id ) {
    // Verify nonce for security
    if (
        ! isset( $_POST['wooq_preorder_nonce'] ) ||
        // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
        ! wp_verify_nonce( wp_unslash( $_POST['wooq_preorder_nonce'] ), 'wooq_preorder_save' )
    ) {
        return;
    }
...
add_action( 'woocommerce_process_product_meta', 'wooqpopskb_preorder_save_product_field' );


/**
 * Change the Add to Cart button text to "Pre-Order" for pre-order products.
 */
function wooqpopskb_preorder_change_button_text( $text, $product ) {
    if ( 'yes' === get_post_meta( $product->get_id(), '_is_preorder', true ) ) {
        return __( 'Pre-Order', 'wooq-preorder' );
    }
    return $text;
}
add_filter( 'woocommerce_product_single_add_to_cart_text', 'wooqpopskb_preorder_change_button_text', 10, 2 );
add_filter( 'woocommerce_product_add_to_cart_text', 'wooqpopskb_preorder_change_button_text', 10, 2 );

/**
 * Allow pre-order products to be purchasable regardless of stock.
 */
function wooqpopskb_preorder_override_purchasable( $purchasable, $product ) {
    if ( 'yes' === get_post_meta( $product->get_id(), '_is_preorder', true ) ) {
        return true;
    }
    return $purchasable;
}
add_filter( 'woocommerce_is_purchasable', 'wooqpopskb_preorder_override_purchasable', 10, 2 );

/**
 * Force pre-order products to be treated as in stock.
 */
function wooqpopskb_preorder_force_in_stock( $in_stock, $product ) {
    if ( 'yes' === get_post_meta( $product->get_id(), '_is_preorder', true ) ) {
        return true;
    }
    return $in_stock;
}
add_filter( 'woocommerce_product_is_in_stock', 'wooqpopskb_preorder_force_in_stock', 10, 2 );

/**
 * Update availability text for pre-order products.
 */
function wooqpopskb_preorder_custom_availability_text( $availability, $product ) {
    if ( 'yes' === get_post_meta( $product->get_id(), '_is_preorder', true ) ) {
        $availability['availability'] = __( 'Pre-Order Available', 'wooq-preorder' );
    }
    return $availability;
}
add_filter( 'woocommerce_get_availability', 'wooqpopskb_preorder_custom_availability_text', 10, 2 );

/**
 * Register custom order status "Pre-Ordered".
 */
function wooqpopskb_register_preorder_status() {
...
add_action( 'init', 'wooqpopskb_register_preorder_status' );

/**
 * Add "Pre-Ordered" to the list of order statuses.
 */
function wooqpopskb_add_preorder_status( $order_statuses ) {
...
add_filter( 'wc_order_statuses', 'wooqpopskb_add_preorder_status' );

/**
 * Set order status to "Pre-Ordered" if any pre-order product is in the order.
 */
function wooqpopskb_set_order_status_preorder( $order_id ) {
    if ( ! $order_id ) {
        return;
    }
...
add_action( 'woocommerce_checkout_order_processed', 'wooqpopskb_set_order_status_preorder', 10, 1 );

function wooqpopskb_maybe_update_order_status_to_pre_ordered( $order_id ) {
    if ( ! $order_id ) {
        return;
    }
    $order = wc_get_order( $order_id );
...
add_action( 'woocommerce_thankyou', 'wooqpopskb_maybe_update_order_status_to_pre_ordered', 20, 1 );

/**
 * Redirect to the cart after adding a pre-order product.
 */
function wooqpopskb_preorder_redirect_to_cart( $url ) {
    if (
        isset( $_REQUEST['add-to-cart'], $_REQUEST['wooq_preorder_nonce'] ) &&
        // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
        wp_verify_nonce( wp_unslash( $_REQUEST['wooq_preorder_nonce'] ), 'wooq_preorder_cart_redirect' )
    ) {
 ...
 */
function wooqpopskb_preorder_inline_js() {
    if ( is_product() ) {
        global $product;
...
add_action( 'wp_footer', 'wooqpopskb_preorder_inline_js' );

/**
 * Override stock quantity for pre-order products.
 */
function wooqpopskb_preorder_override_stock_quantity( $quantity, $product ) {
    if ( 'yes' === get_post_meta( $product->get_id(), '_is_preorder', true ) ) {
...
add_filter( 'woocommerce_product_get_stock_quantity', 'wooqpopskb_preorder_override_stock_quantity', 10, 2 );

/**
 * Add a warning message under the product title on the cart page if pre-ordered.
 */
function wooqpopskb_preorder_cart_warning( $item_name, $cart_item, $cart_item_key ) {
...
add_filter( 'woocommerce_cart_item_name', 'wooqpopskb_preorder_cart_warning', 10, 3 );

/**
 * Admin inline script to toggle the Pre-Order Stock Quantity field.
 */
function wooqpopskb_preorder_admin_script() {
    ?>
    <script type="text/javascript">
    jQuery(document).ready(function($){
...
add_action( 'admin_footer', 'wooqpopskb_preorder_admin_script' );

/**
 * Override stock checks on checkout for pre-order products.
 */
function wooqpopskb_preorder_override_stock_check( $has_enough, $product, $quantity ) {
    if ( 'yes' === get_post_meta( $product->get_id(), '_is_preorder', true ) ) {
        return true;
    }
    return $has_enough;
}
add_filter( 'woocommerce_product_has_enough_stock', 'wooqpopskb_preorder_override_stock_check', 10, 3 );
add_filter( 'woocommerce_variation_has_enough_stock', 'wooqpopskb_preorder_override_stock_check', 10, 3 );

/**
 * Allow backorders for pre-order products.
 */
function wooqpopskb_preorder_allow_backorders( $allowed, $product ) {
...
add_filter( 'woocommerce_product_backorders_allowed', 'wooqpopskb_preorder_allow_backorders', 10, 2 );

/**
 * Display a note on the order details page for pre-ordered products.
 */
function wooqpopskb_preorder_order_item_meta( $item_id, $item, $order, $plain_text ) {
    $product = $item->get_product();
    if ( $product && 'yes' === get_post_meta( $product->get_id(), '_is_preorder', true ) ) {
        if ( $plain_text ) {
            echo "\n" . esc_html__( 'This product is pre-ordered.', 'wooq-preorder' );
        } else {
            echo '<p style="color: red; font-weight: bold; margin: 0;">' . esc_html__( 'This product is pre-ordered.', 'wooq-preorder' ) . '</p>';
        }
    }
}
add_action( 'woocommerce_order_item_meta_end', 'wooqpopskb_preorder_order_item_meta', 10, 4 );

/**
 * Hide the "Buy Now" button and the "OR" wrapper on product pages if pre-order is enabled.
 */
function wooqpopskb_hide_buy_now_and_or_if_preorder() {
    if ( is_product() ) {
        global $product;
        if ( $product && 'yes' === get_post_meta( $product->get_id(), '_is_preorder', true ) ) {
            echo '<style>
...
add_action( 'wp_head', 'wooqpopskb_hide_buy_now_and_or_if_preorder' );
